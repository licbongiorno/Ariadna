...
// Trampa para evitar bloqueo HTML5
['instagram', 'linkedin', 'facebook', 'web'].forEach(id => { const el = document.getElementById(id); if(el) el.type = 'text'; });
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        if (loginIntent) { await setDoc(doc(db, "users", user.uid), { role: loginIntent, email: user.email }, { merge: true }); loginIntent = ''; }
        const userDoc = await getDoc(doc(db, "users", user.uid));
        if (userDoc.exists()) {
            const data = userDoc.data(); currentRole = data.role;
            if(currentRole === 'paciente') { datosPaciente.nombre = data.nombrePaciente || ''; datosPaciente.motivo = data.motivoPaciente || ''; favoritos = data.favoritos || []; }
...
// ==========================================
// 5. MAPA DEL FORMULARIO DE REGISTRO
// ==========================================
function initFormMap(lat = -31.6529, lng = -64.4283) {
    setTimeout(() => {
        if (formMapInstance) { formMapInstance.remove(); formMapInstance = null; }
        ...
        if(lat !== -31.6529 && lng !== -64.4283) { colocarMarcadorArrastrable(lat, lng); } else { formMarker = null; document.getElementById('form-lat').value = ''; document.getElementById('form-lng').value = ''; }
...
function colocarMarcadorArrastrable(lat, lng) {
    ...
    formMarker.on('dragend', function(e) { const position = formMarker.getLatLng(); document.getElementById('form-lat').value = position.lat; document.getElementById('form-lng').value = position.lng; });
}

document.getElementById('btn-reset-map').addEventListener('click', () => { if(formMarker && formMapInstance) formMapInstance.removeLayer(formMarker); document.getElementById('form-lat').value = ''; document.getElementById('form-lng').value = ''; });

async function obtenerCoordenadas(direccion) {
    try {
        const query = encodeURIComponent(direccion + ", Córdoba, Argentina");
        const url = `https://nominatim.openstreetmap.org/search?format=json&q=${query}&limit=1`;
        const res = await fetch(url); if(!res.ok) throw new Error("Fallo API Mapa");
...
if(btnBuscarDir) {
    btnBuscarDir.addEventListener('click', async () => {
        ...
        if(coords) { formMapInstance.setView([coords.lat, coords.lng], 16); colocarMarcadorArrastrable(coords.lat, coords.lng); showToast("Ubicación encontrada."); } 
...
function formatUrl(url) {
    if (!url) return null; let clean = url.trim(); if (clean === "") return null;
    if (!clean.startsWith('http://') && !clean.startsWith('https://')) return 'https://' + clean;
    return clean;
}
...
formProfesional.addEventListener('submit', async (e) => {
    ...
    btnSubmit.textContent = "Guardando información...";
    ...
    if (formLat && formLng) { coordsGuardar = { lat: parseFloat(formLat), lng: parseFloat(formLng) }; } 
...
function getWhatsAppLink(numeroProf) { let mensaje = "Hola, te contacto desde Ariadna."; if (datosPaciente.nombre) { mensaje = `Hola, mi nombre es ${datosPaciente.nombre}. Te contacto desde Ariadna.`; } return `https://wa.me/${numeroProf}?text=${encodeURIComponent(mensaje)}`; }
...
function renderCards(data) {
    ...
    data.forEach((prof, index) => {
        ...
        let imagenHtml = `<div class="avatar" style="background-color: ${avatar.color}">${avatar.initials}</div>`;
...
function abrirFichaProfesional(id) {
    ...
    if (tieneUbicacion) {
        let destinoRuta = '';
        if (prof.coordenadas && prof.coordenadas.lat) { destinoRuta = `${prof.coordenadas.lat},${prof.coordenadas.lng}`; } 
        else if (dirList.length > 0) { destinoRuta = encodeURIComponent(dirList[0] + ', Córdoba, Argentina'); }
    ...
    let imagenFichaHtml = `<div class="avatar" style="background-color: ${avatar.color}; width: 60px; height: 60px; font-size: 1.5rem;">${avatar.initials}</div>`;

    fichaBody.innerHTML = `
        <div class="ficha-perfil">
            <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 1rem;">
                ${imagenFichaHtml}
                <div><span class="prof-category">${prof.categoria}</span><h3 style="margin: 0;">${prof.nombre}</h3><p style="margin: 0; font-size: 0.85rem; color: var(--text-muted);"><strong>Matrícula:</strong> ${prof.matricula}</p></div>
            </div>
...
function getFilteredProfesionales() {
    ...
    const textoLimpio = quitarTildes(textoOriginal); 
    
    const categoria = document.getElementById('select-category') ? document.getElementById('select-category').value : 'Todas';
    const modalidadFiltro = document.getElementById('select-modalidad') ? document.getElementById('select-modalidad').value : 'Todas';
    const publicoFiltro = document.getElementById('select-publico') ? document.getElementById('select-publico').value : 'Todos';

    return profesionalesCache.filter(prof => {
        // Filtro 1: Especialidad
        const matchCategoria = categoria === 'Todas' || prof.categoria === categoria; 
        
        // Filtro 2: Modalidad Cruzada Inteligente
        let matchModalidad = true;
        const mod = (prof.modalidad || '').toLowerCase();
        
        if (modalidadFiltro === 'Online') matchModalidad = mod.includes('online') || mod.includes('ambas');
        if (modalidadFiltro === 'Presencial') matchModalidad = mod.includes('presencial') || mod.includes('ambas');
        if (modalidadFiltro === 'Ambas') matchModalidad = mod.includes('ambas');
        if (modalidadFiltro === 'A domicilio') matchModalidad = mod.includes('domicilio');

        // Filtro 3: Público (Edades)
        let matchPublico = true;
        if (publicoFiltro !== 'Todos') {
            if (prof.atencion) { matchPublico = prof.atencion[publicoFiltro] === true; } 
            else { matchPublico = false; }
        }

        // Filtro 4: Búsqueda de texto normal
        let matchTexto = true;
        if (textoLimpio !== '') {
            ...
            matchTexto = quitarTildes(bolsaDePalabras.join(' ').toLowerCase()).includes(textoLimpio);
        }

        return matchCategoria && matchModalidad && matchPublico && matchTexto;
    });
}

function aplicarFiltros() { const filtrados = getFilteredProfesionales(); renderCards(filtrados); }

// Escuchadores de eventos para los filtros
document.getElementById('input-search').addEventListener('input', aplicarFiltros); 
if(document.getElementById('select-category')) document.getElementById('select-category').addEventListener('change', aplicarFiltros);
if(document.getElementById('select-modalidad')) document.getElementById('select-modalidad').addEventListener('change', aplicarFiltros);
if(document.getElementById('select-publico')) document.getElementById('select-publico').addEventListener('change', aplicarFiltros);
